...
{
    QString path = BootSettings::userResourcePath();
...
void FontSettingsPage::refreshColorSchemeList()
{
    ...
    QString resourcePath = BootSettings::resourcePath();
...
void FontSettingsPage::saveSettings()
{
    if (d_ptr->m_value != d_ptr->m_lastValue) {
        d_ptr->m_lastValue = d_ptr->m_value;
        d_ptr->m_value.toSettings(d_ptr->m_settingsGroup, BootSettings::settings());
