...
#define COMPONENT_NAME "FolderListModel"
...
{
    QString localPath = urlToLocalFileOrQrc(path);
...
empty string.
*/
QString QQuickFolderListModelPrivate::urlToLocalFileOrQrc(const QUrl& url)
{
...
QStringList QQuickFolderListModel::hiddenSuffixes() const
{
	Q_D(const QQuickFolderListModel);
	return d->hiddenSuffixes;
}

void QQuickFolderListModel::setHiddenSuffixes(const QStringList& hiddenSuffixes)
{
	Q_D(QQuickFolderListModel);
	d->fileInfoThread.setHiddenSuffixes(hiddenSuffixes);
	d->hiddenSuffixes = hiddenSuffixes;
}
...
{
    Q_D(QQuickFolderListModel);
    QString localPath = QQuickFolderListModelPrivate::urlToLocalFileOrQrc(d->currentDir);
...
void QQuickFolderListModel::registerQmlType()
{
	qmlRegisterType<QQuickFolderListModel>(COMPONENT_URI, COMPONENT_VERSION_MAJOR, COMPONENT_VERSION_MINOR, COMPONENT_NAME);
}
