...
TextDocumentManipulator::TextDocumentManipulator(QmlCodeEditor *textEditorWidget)
...
{
    return m_textEditorWidget->codeDocument()->characterAt(position);
}

QString TextDocumentManipulator::textAt(int position, int length) const
{
    return m_textEditorWidget->codeDocument()->textAt(position, length);
}
...
void TextDocumentManipulator::autoIndent(int position, int length)
{
    ...
    if (hasOnlyBlanksBeforeCursorInLine(cursor)) {
        ...
        m_textEditorWidget->codeDocument()->autoIndent(cursor);
    }
}

bool TextDocumentManipulator::textIsDifferentAt(int position, int length, const QString &text) const
{
    const auto textToBeReplaced = m_textEditorWidget->codeDocument()->textAt(position, length);
