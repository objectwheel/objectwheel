...
QString FilePath::toString() const
{
    ...
    if (pathView().isEmpty())
        return  (u"://"_s + encodedHost()).prepend(scheme());

    if (isRelativePath())
        return (u"://"_s + encodedHost() + u"/./"_s).append(pathView()).prepend(scheme());
    return (u"://"_s + encodedHost()).append(pathView()).prepend(scheme());
}
...
QString FilePath::toFSPathString() const
{
    ...
    if (pathView().isEmpty())
        return (specialRootPath() + '/').append(scheme()) + '/' + encodedHost();

    if (isRelativePath())
        return ((specialRootPath() + '/').append(scheme()) + '/' + encodedHost() + "/./").append(pathView());
    return ((specialRootPath() + '/').append(scheme()) + '/' + encodedHost()).append(pathView());
}
...
{
    static const QString rootPath = QDir::rootPath() + u"__qtc_devices__"_s;
    return rootPath;
}
...
{
    static const QString deviceRootPath =  QDir::rootPath() + u"__qtc_devices__/device"_s;
...
{
    static const QStringView qtcDevSlash(u"__qtc_devices__/"_s);
    static const QStringView colonSlashSlash(u"://"_s);
...
UTILS_EXPORT bool operator==(const FilePath &first, const FilePath &second)
{
...
UTILS_EXPORT size_t qHash(const FilePath &filePath, uint seed)
{
    Q_UNUSED(seed);
...
UTILS_EXPORT size_t qHash(const FilePath &filePath)
{
    return qHash(filePath, 0);
}

UTILS_EXPORT QDebug operator<<(QDebug dbg, const FilePath &c)
{
